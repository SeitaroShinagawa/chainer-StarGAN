...
class Updater(chainer.training.StandardUpdater):
    ...
    def update_core(self):
        ...
        for i in range(self._n_dis):
            ...
            out_fake_forD, out_fake_att_forD = self.dis(y_fake_copy) #for Discriminator learning 
            patch_size = y_fake.data.shape[-1]

            #Generator learning
            if i == 0:
                loss_gen_adv = F.sum(-out_fake) / batchsize / patch_size**2
                loss_gen_cls = F.sigmoid_cross_entropy(out_fake_att,Variable(y_att)) #CelebA requires
                ...
                loss_gen.backward()
                opt_gen.update()
                chainer.reporter.report({'loss_gen_fake': loss_gen_adv})
            ...
            #loss_gp = F.mean_squared_error(dydx, 100 * xp.ones_like(dydx.data)) * (1.0/100**2)

            loss_dis_real = F.sum(-out_real) / batchsize / patch_size**2
            loss_dis_fake = F.sum(out_fake_forD) / batchsize / patch_size**2
            loss_dis_adv = loss_dis_real + loss_dis_fake + self._lambda_gp * loss_gp
            ...
            chainer.reporter.report({'loss_dis_real': loss_dis_real})
            chainer.reporter.report({'loss_dis_fake': loss_dis_fake})
