...
def batch_postprocess_images(img, batch_w, batch_h):
    ...
    img = img.reshape((batch_w, batch_h, w, h, ch)).transpose(0,2,1,3,4).reshape((w*batch_w, h*batch_h, ch))[:,:,::-1] #なんでこいつだけBGR -> RGBしてんの？
    return img
...
def evaluation(gen, test_image_folder, image_size=128, side=2):
    ...
    def _eval(trainer, it):
        ...
        for i in range(2**gen.nc_size):
            ...
            #x = xp.broadcast_to(x_origin,(att.shape[0],batchsize,3,image_size,image_size))
            #x = x.swapaxes(0,1).reshape(batchsize*att.shape[0],3,image_size,image_size)
            #att = xp.broadcast_to(att,(batchsize,att.shape[0],att.shape[1])).reshape(batchsize*att.shape[0],att.shape[1])
